	...
	a.extend(a.jqx.mobile, {
		...
		touchScroll : function(c, t, e, u) {
			...
			n.bind("touchstart.touchScroll", function(w) {
				...
				var x = v.getTouches(w);
				var y = x[0];
				if (x.length == 1) {
					v.dispatchMouseEvent("mousedown", y, v
			...
			if (this.simulatetouches) {
				...
				a(document).bind("touchend", function(w) {
					...
					m = false;
					var y = v.getTouches(w)[0], x = v.getRootNode(y.target);
					v.dispatchMouseEvent("mouseup", y, x);
					v.dispatchMouseEvent("click", y, x)
				})
			}
			n.bind("touchend.touchScroll touchcancel.touchScroll", function(w) {
				var y = v.getTouches(w)[0];
				if (!m) {
					return true
				}
				m = false;
